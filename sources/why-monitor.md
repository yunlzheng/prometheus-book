...
通过监控了解各种资源的当前使用情况以及增长速率，了。这些数据可以帮助用户了解对未来一段时间内的趋势进行预判。 例如，通过对磁盘空间增长率的判断，我们可以提前预测在未来什么时间节点上需要对资源进行扩容。
...
对于上一代监控系统(例如：Nagios、Zabbix)的用户而言往往并不能很好的解决上述问题。这里以Nagios为例，如下图所示：
